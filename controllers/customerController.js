...
const registerCustomer = async (req, res) => {
    try {
        ...
        if (resp.success) {
            ...
            res.status(201).json({status: 200, success: true, message: "Customer registered. OTP sent to you phone number." });
        } else {
            res.status(500).json({ status: 500, success: false, error: "OTP not sent" });
        }
...
const verifyOTP = async (req, res) => {
    try {
        ...
        if (existingCustomer.otp && existingCustomer.otp != otp) {
            return res.status(400).json({ status: 400, success: false, message: "Invalid or expired OTP!" });
        }
...
const updateCustomer = async (req, res) => {
    try {
        const { customerId, username, phone_number, address, isEmailVerified, isPhoneVerified } = req.body;

        const updatedCustomer = await Customers.Customer.findByIdAndUpdate(
            customerId,
            { username, phone_number, address, isEmailVerified, isPhoneVerified },
            { new: true }
        );

        if (!updatedCustomer) return res.status(404).json({ status: 404, success: false, message: "Customer not found!" });

        res.json({ status: 100, success: true, message: "Customer updated!", customer: updatedCustomer });
    } catch (error) {
        res.status(500).json({ status: 500, success: false, error: error.message });
    }
};
...
const deleteCustomer = async (req, res) => {
    try {
        ...
        res.json({ status: 200, success: true, message: "Customer deleted!" });
    } catch (error) {
        res.status(500).json({ status: 500, success: false, error: error.message });
    }
};
